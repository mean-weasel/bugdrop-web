...
async function openRawPayload(page: Page, launcherName: string) {
  ...
  await expect(link).toHaveAttribute("href", /\/labs\/variants\/submissions\/\d+$/);
  const [viewer] = await Promise.all([page.context().waitForEvent("page"), link.click()]);
  ...
  const payload = JSON.parse((await raw.textContent()) ?? "null") as Record<string, unknown>;
...
test("both pinned public flows submit only to inspectable local payloads", async ({
  ...
  await page.goto(`/labs/variants?sentinel=${querySentinel}#${fragmentSentinel}`);
  ...
  await expect(page.getByRole("button", { name: "Run product-triage flow" })).toBeEnabled();

  await page.getByRole("button", { name: "Run default-shaped flow" }).click();
  const defaultFlow = page.locator('[data-bugdrop-flow="lab-default-shaped-flow"]');
  await expect(defaultFlow.getByRole("dialog", { name: "Share feedback" })).toBeVisible();
  ...
  await defaultFlow.getByLabel("Description").fill("Stored by the website inspector.");
  ...
  await expect(defaultFlow.getByRole("heading", { name: "Thanks for your feedback!" })).toBeVisible();
  await expect(defaultFlow.getByRole("link")).toHaveCount(0);
  await defaultFlow.getByRole("button", { name: "Done" }).click();
  await expect(page.getByRole("button", { name: "Run default-shaped flow" })).toBeFocused();
  ...
  const triageFlow = page.locator('[data-bugdrop-flow="lab-product-triage-flow"]');
  await expect(triageFlow.getByRole("dialog", { name: "Help us prioritize" })).toBeVisible();
  ...
  await triageFlow.getByRole("button", { name: "Continue" }).click();
  await triageFlow.getByLabel("Steps to reproduce").fill("stale-hidden-answer-sentinel");
  await triageFlow.getByRole("button", { name: "Continue" }).click();
  ...
  await expect(triageFlow.getByRole("heading", { name: "Thanks for your feedback!" })).toBeVisible();
  await expect(triageFlow.getByRole("link")).toHaveCount(0);
  await triageFlow.getByRole("button", { name: "Done" }).click();
  await expect(page.getByRole("button", { name: "Run product-triage flow" })).toBeFocused();
...
test("the public runtime section and dialog remain accessible at mobile width", async ({
  ...
  const heading = page.getByRole("heading", { name: "Run the real FlowConfig examples" });
