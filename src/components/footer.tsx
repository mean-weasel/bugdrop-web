    ...
    <footer data-site-footer className="mt-20 border-t border-border/80 pt-12 max-sm:mt-14 max-sm:pt-9">
      <div className="grid grid-cols-[minmax(0,1.6fr)_repeat(3,minmax(0,1fr))] gap-x-10 gap-y-10 max-lg:grid-cols-2">
        <div className="max-w-sm max-lg:col-span-2">
          <Link href="/" className="inline-flex items-center text-lg font-semibold text-text-primary no-underline">
            BugDrop
          </Link>
          ...
          <a
            ...
            className="mt-5 inline-flex items-center gap-2 text-xs font-medium text-text-subtle no-underline transition-colors hover:text-accent-warm"
          >
        ...
        {footerGroups.map((group) => (
          <nav key={group.title} aria-label={`${group.title} links`}>
            <h2 className="text-xs font-semibold uppercase tracking-[0.16em] text-text-subtle">
              {group.title}
            </h2>
      ...
      <div className="mt-12 flex items-center justify-between gap-5 border-t border-border/70 py-6 text-xs text-text-muted max-sm:flex-col max-sm:items-start">
        <p>
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="text-text-subtle no-underline transition-colors hover:text-accent-cyan"
          >
            neonwatty
          </a>{" "}
          and{" "}
        ...
        <a
          ...
          className="text-text-subtle no-underline transition-colors hover:text-accent-cyan"
        >
          More independent software <span aria-hidden="true">↗</span>
          <span className="sr-only"> (opens in a new tab)</span>
        </a>
      </div>
    </footer>
