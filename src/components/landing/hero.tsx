...
export async function Hero() {
  ...
  return (
    <header className={styles["hero-layout"]}>
      <div className="hero-copy">
        ...
        <h1 className={styles["hero-title"]}>Website feedback to GitHub Issues - <strong>with screenshots.</strong></h1>
        <p className={styles["hero-description"]}><strong className="font-semibold text-text-primary">No new dashboard.</strong> Get bug reports, annotated screenshots, and browser details right in GitHub - where your team already works.</p>
        <div className={styles["hero-actions"]}>
          <a href="#flows" aria-describedby="hero-demo-note" data-homepage-hero-activate data-analytics-event="landing_cta_click" data-analytics-label="Try the widget" className={styles["hero-primary"]}>
            <MessageSquare size={17} aria-hidden="true" /> Try the widget
          </a>
          <a href={MARKETPLACE_URL} target="_blank" rel="noopener noreferrer" data-analytics-event="outbound_marketplace_click" data-analytics-label="Install from GitHub Marketplace" className={styles["hero-install"]}>
      ...
      <div className={styles["hero-proof"]}>
        ...
        <a href={GITHUB_REPO_URL} target="_blank" rel="noopener noreferrer" className={styles["hero-source"]}>Explore the source <ArrowUpRight size={14} aria-hidden="true" /></a>
      </div>
    </header>
  );
}
