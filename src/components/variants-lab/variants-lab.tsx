...
export function VariantsLab() {
  ...
  function reset() {
    ...
    setValidationField(null);
    requestAnimationFrame(() => fieldRefs.current[recipe.fields[0].id]?.focus());
  }
    ...
    if (field.type === "shortText" || field.type === "longText") {
      ...
      return (
        <label className={styles.textField} key={field.id}>
          ...
          {field.type === "longText" ? <textarea {...shared} /> : <input {...shared} />}
          {invalid ? <em id={errorId}>Complete {field.label.toLowerCase()} before submitting.</em> : null}
        </label>
      );
    }
    ...
    if (field.type === "singleChoice") {
      return (
        <fieldset className={styles.fieldset} key={field.id} aria-invalid={invalid || undefined} aria-describedby={invalid ? errorId : undefined}>
          <legend>{field.label}{field.required ? <b>Required</b> : null}</legend>
          <div className={styles.choiceList}>{field.options.map((option, index) => (
            <label key={option.value}><input ref={(element) => { if (index === 0) fieldRefs.current[field.id] = element; }} type="radio" name={`${recipeId}-${field.id}`} value={option.value} checked={interaction.answers[field.id] === option.value} disabled={disabled} onChange={() => update(field.id, option.value)} /><span>{option.label}</span></label>
          ))}</div>
          {invalid ? <em id={errorId}>Choose one option before submitting.</em> : null}
        </fieldset>
      );
    }
    const selected = Array.isArray(interaction.answers[field.id]) ? interaction.answers[field.id] as string[] : [];
    return (
      <fieldset className={styles.fieldset} key={field.id} aria-invalid={invalid || undefined} aria-describedby={invalid ? errorId : undefined}>
        <legend>{field.label}{field.required ? <b>Required</b> : null}</legend>
        <div className={styles.choiceList}>{field.options.map((option, index) => (
          <label key={option.value}><input ref={(element) => { if (index === 0) fieldRefs.current[field.id] = element; }} type="checkbox" value={option.value} checked={selected.includes(option.value)} disabled={disabled} onChange={(event) => update(field.id, event.target.checked ? [...selected, option.value] : selected.filter((value) => value !== option.value))} /><span>{option.label}</span></label>
        ))}</div>
        {invalid ? <em id={errorId}>Choose at least one option before submitting.</em> : null}
      </fieldset>
    );
  }
  ...
  const form = (
    <div className={styles.formBody}>
      ...
      {interaction.status === "failure" ? <div className={styles.failure} role="alert"><strong>{FAILURE_MESSAGE}</strong><span>Retry the explicit submit to preview success.</span></div> : null}
      {interaction.status === "success" ? <div ref={successRef} className={styles.success} role="status" tabIndex={-1}><Check aria-hidden="true" /><div><strong>{recipe.content.successTitle}</strong><span>{recipe.content.successMessage}</span></div></div> : null}
      <div className={styles.actions}>
        {interaction.status !== "success" ? <button type="button" className={styles.primaryButton} onClick={submit}>{interaction.status === "failure" ? "Retry local preview" : recipe.content.submitLabel}</button> : null}
        <button type="button" className={styles.secondaryButton} onClick={reset}><RotateCcw aria-hidden="true" /> Reset recipe</button>
      </div>
    </div>
  );
  ...
  return (
    <main className={styles.lab}>
      <header className={styles.hero}>
        <span className={styles.labTag}><FlaskConical aria-hidden="true" /> Composable feedback</span>
      ...
      <section className={styles.explorer} aria-labelledby="blocks-heading">
        <header className={styles.sectionHeader}><h2 id="blocks-heading">Start with a building block</h2></header>
        <ul className={styles.primitiveList}>
          {PRIMITIVE_IDS.map((primitive) => <li key={primitive}><strong>{PRIMITIVE_COPY[primitive].label}</strong><span>{PRIMITIVE_COPY[primitive].note}</span></li>)}
        </ul>
        <div className={styles.recipePicker}>
          <span>Try a combination</span>
          <div className={styles.recipeTabs}>
            {RECIPE_IDS.map((id) => (
              <button key={id} type="button" aria-pressed={recipeId === id} onClick={() => chooseRecipe(id)}>
                <strong>{RECIPES[id].name}</strong>
                <small>{RECIPES[id].fields.map((field) => PRIMITIVE_COPY[field.type].label).join(" + ")}</small>
              </button>
            ))}
          </div>
        </div>
      ...
      <section className={styles.workspace} aria-labelledby="workspace-heading">
        <header className={styles.workspaceHeader}>
          <div><span>Live example</span><h2 id="workspace-heading" ref={workspaceRef} tabIndex={-1}>{recipe.name}</h2></div>
          <p className={styles.disclosure}><ShieldCheck aria-hidden="true" /> {LAB_DISCLOSURE}</p>
        </header>
        <div className={styles.workspaceGrid}>
          <div className={styles.surface}>
            <div className={styles.widget}>
              ...
              {recipe.presentation.kind === "modal" ? <button ref={modalTriggerRef} type="button" className={styles.primaryButton} onClick={() => dialogRef.current?.showModal()}>Open feedback recipe</button> : form}
            </div>
          </div>
          <aside className={styles.issuePreview} aria-label="Local GitHub Issue preview">
            <div className={styles.previewHeader}><div><span>Local GitHub Issue</span><strong>Issue preview</strong></div><b>Not created</b></div>
            {issue.ok ? <><h3>{issue.draft.title}</h3><div className={styles.labels}>{issue.draft.labels.map((label) => <span key={label}>{label}</span>)}</div><pre>{issue.draft.body}</pre><small>Local preview only — nothing was submitted.</small></> : <div className={styles.previewEmpty}><strong>Complete the required blocks</strong><span>The Issue preview will compile here as you answer.</span></div>}
          </aside>
        </div>
      </section>
      ...
      <section className={styles.requestBlock}>
        ...
        <a href="https://github.com/mean-weasel/bugdrop/issues/new">Request one on GitHub <ArrowUpRight aria-hidden="true" /></a>
      </section>
