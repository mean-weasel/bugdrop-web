...
| `data-welcome` | `"once"` | Welcome screen behavior: `"once"`, `"always"`, or `"never"` |
...
- **`auto`** (default) -- Follows the visitor's system color scheme.
...
| `data-dismiss-duration` | *(forever)* | Positive whole number of days before the dismissed button can reappear |
| `data-show-restore` | `"true"` | Show a small restore link after dismissing |

### Dismissible Button Example

```html
<!-- Dismissible button that stays hidden for 30 days -->
<script
  src="https://bugdrop.neonwatty.workers.dev/widget.js"
  data-repo="owner/repo"
...
3. The button stays hidden for the number of days specified by `data-dismiss-duration`, or forever if no duration is set
...
This configuration creates a dark-themed widget in the bottom-left corner with an indigo accent color, collects the submitter's name and email (email required), and shows a "Feedback" label on the button. The button is dismissible for 30 days with a restore link.
