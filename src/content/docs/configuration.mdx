...
BugDrop preserves screenshots at their captured resolution when they fit the Worker's upload limit. The widget learns that limit during its installation check and attempts to scale larger captured or annotated PNGs down before submission. Self-hosted Workers use `MAX_SCREENSHOT_SIZE_MB` to set the limit; it defaults to 5 MB.
