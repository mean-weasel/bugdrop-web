...
import { POST } from "@/app/vendor/bugdrop/b307db46d895ae141f34befbc8cfcd107b155bc9/api/feedback/route";
import {
...
describe("public runtime lab boundary", () => {
  ...
  it("returns a private local result with a canonical synthetic Issue URL", async () => {
    ...
    const response = await POST(
      new Request(
        "http://bugdrop.localhost:3000/vendor/bugdrop/b307db46d895ae141f34befbc8cfcd107b155bc9/api/feedback",
        {
          method: "POST",
          headers: {
  ...
  it("evicts older records to keep retained payload bytes bounded", () => {
    createLocalSubmission({ value: "first" }, 40 * 1024 * 1024);
    createLocalSubmission({ value: "second" }, 40 * 1024 * 1024);

    expect(getLocalSubmission(1)).toBeNull();
    expect(getLocalSubmission(2)?.payload).toEqual({ value: "second" });
  });
